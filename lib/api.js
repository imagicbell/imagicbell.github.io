...
import { Settings, EXCERPT_LENGTH_PREVIEW, EXCERPT_LENGTH_CN_PREVIEW } from './constants';


const postsDirectory = join(process.cwd(), '_local/posts');
...
export function getPostForPreview(slug) {
  ...
  if (post.excerpt === undefined) {
    post.excerpt = extractPostExcerpt(post, { en: EXCERPT_LENGTH_PREVIEW, cn: EXCERPT_LENGTH_CN_PREVIEW });
  }
  delete post.content;
  return post;  
} 

export function extractPostExcerpt(post, length) {
...
export function calcMarkdownReadTime(content, locale) {
  let time = Math.floor(countMarkdownText(content, locale) / (locale === 'cn' ? 500 : 200));
